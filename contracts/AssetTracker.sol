...
contract AssetTracker {
    ...
    function getAsset(uint _id) view public returns(string memory, string memory, string memory, string memory, string memory, string memory) {
        return (AssetStore[_id].batchNo, AssetStore[_id].name, AssetStore[_id].description, AssetStore[_id].manufacturer, AssetStore[_id].status[AssetStore[_id].statusCount].owner, AssetStore[_id].status[AssetStore[_id].statusCount].status);
    }
    
    function transferAsset(uint _id, string memory _newOwner, string memory _status) public{
